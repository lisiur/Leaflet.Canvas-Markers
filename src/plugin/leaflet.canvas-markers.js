...
function layerFactory(L) {
    var CanvasIconLayer = (L.Layer ? L.Layer : L.Class).extend({
        ...
        _drawMarker: function(marker, pointPos) {
            ...
            } else if (marker.options.icon.options.html) {
                self._drawHtml(marker, pointPos)
            } else {
        ...
        _drawHtml: function(marker, pointPos) {
            var options = marker.options.icon.options
            var iconFont = createElementFromHTML(options.html)
            var fontSize = iconFont.style['font-size']
            var color = iconFont.style.color
            var className = iconFont.className

            this._context.font = `${fontSize} ${className}`
